...
def comboajax(request, modelo, foreignkey, fieldkey):
  """
  Preenche o elemento <select> de destino de acordo com o item selecionado
  no <select> de origem.
  ...
  Parametros
    ...
    fiedlkey - Campo cujo valor sera exibido no <select> de destino

  Exemplo de uso:

    /*
     * Retorna lista de cidades a partir de um estado selecionado
     */
    function comboAjax() {
      $('#cities').html('<option value="0">Wait...</option>');
      $.ajax({
        type: 'POST',
        url: '/client/ajax/comboajax/City/state/cityname',
        dataType: 'json',
        data: { 'id': 1 },
        success: function(retorno) {
          $('#cities').empty();
          $.each(retorno, function(i, item) {
            $('#cities').append('<option value="'+item.pk+'">'+item.fields['cityname']+'</option>');
          });
        }
      });
    }
